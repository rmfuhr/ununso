...
(function($){
    ...
    // init fullscreen();
    fullscreen();
    // init double menu for shop/shop3
    top_menu_height();
    // init fitscreen function
    if ($(window).width() > 767) {
        // init fitscreen function
        introContentHeight();
    }
...
/*
    ...
    $(".tweet-feed").twittie({
        dateFormat: "%b. %d, %Y",
        template: '{{tweet}} <span class="twieet-date"><i class="fa fa-clock-o"></i> {{date}}</span>',
        count: 2
    });
        ...
        $("#footer_switch").on('change',function(){
            ...
            if(footerTypes.indexOf(footer)>=0) {
                $("#footer_canvas").load(footer+".php",function() {
                    ...
                    //init twitter feed after widget change
                    $(".tweet-feed").twittie({
                        dateFormat: "%b. %d, %Y",
                        template: '{{tweet}} <span class="twieet-date"><i class="fa fa-clock-o"></i> {{date}}</span>',
                        count: 2
...
// Twitter Follow Window
function twitterFollow () {
    window.open("https://twitter.com/pixelcoderz", "", "width=560, height=400");
}
function fitScreen () {
    // window width and height
    var display = {
    ...
    // fitscreen, window width and height
    // without height 
    var fitscreen = display.dheight - headerheight;
    // universal fitscreen class
